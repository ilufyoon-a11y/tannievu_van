...
MAX_FALLOS = len(ETAPAS_AHORCADO) - 1  # 6 fallos = pierden
...
sesion_ahorcado = {
    ...
    "letras_incorrectas": set(),   # letras ya mencionadas por cualquiera (para no repetirlas)
    "moderador_id": None,
...
esperando_palabra_ahorcado = {}  # user_id -> chat_id
...
async def recibir_palabra_ahorcado(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, texto: str):
    ...
    await context.bot.send_message(
        ...
        text=(f"🪢 **¡AHORCADO INICIADO!**\n\n"
              ...
              f"`{pantalla}`\n\n"
              f"{ETAPAS_AHORCADO[0]}\n"
              f"❤️ Intentos restantes: {MAX_FALLOS}\n\n"
...
async def escuchar_ahorcado_grupo(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, texto: str, chat_id: int):
    ...
    if user_id in sesion_ahorcado.get("eliminados", set()):
        return  # ya gastó sus intentos, no puede seguir participando
    if not texto:
        return
    ...
    if intento in palabra:
        ...
        if vidas_restantes <= 0:
            ...
            # Si ya no queda nadie con intentos disponibles, se acaba la ronda
